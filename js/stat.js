...
(function () {
  window.renderStatistics = function (ctx, names, times) {
    ...
    var drawStatistic = function () {
      ...
      var step = HISTOGRAM_HEIGHT / window.util.getMax(times);

      for (var i = 0; i < times.length; i++) {
        var columnHeight = times[i] * step;
        var coordX = INITIAL_COORD_X + (COLUMN_INDENT + COLUMN_WIDTH) * i;
        var coordY = INITIAL_COORD_Y - columnHeight;
        ctx.fillStyle = names[i] === YOUR_NAME ?
          YOUR_COLOR : COLOR_BEFORE_ALPHA + Math.random() + COLOR_AFTER_ALPHA;

        ctx.fillRect(coordX, coordY, COLUMN_WIDTH, columnHeight);
        ctx.fillStyle = COLOR_BLACK;
        ctx.fillText(names[i], coordX, INITIAL_COORD_Y + NAME_INDENT);
        ctx.fillText(Math.round(times[i]), coordX, coordY - POINTS_INDENT);
      }
    };

    drawCloud(SHADOW_COORD_X, SHADOW_COORD_Y, CLOUD_WIDTH,
        CLOUD_HEIGHT, SHADOW_COLOR);
    drawCloud(CLOUD_COORD_X, CLOUD_COORD_Y, CLOUD_WIDTH,
        CLOUD_HEIGHT, CLOUD_COLOR);
    drawStatistic();
  };
})();
